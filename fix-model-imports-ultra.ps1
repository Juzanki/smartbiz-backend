# fix-model-imports-ultra.ps1
param(
  [string]$Root = "E:\SmartBiz_Assistance\backend"
)

Write-Host "ULTRA pass: scanning and fixing remaining barrel imports..." -ForegroundColor Cyan

$ModelsDir = Join-Path $Root "models"
if (-not (Test-Path $ModelsDir)) { throw "Models dir not found: $ModelsDir" }

# Regex: kubaini class ikiwepo na '(' AU ':' baada ya jina
function New-ClassRegex([string]$name) {
  return [regex]("(?m)^\s*class\s+" + [regex]::Escape($name) + "\s*(?:[\(:])")
}

# Jenga index ya Class -> module kutoka models/*.py
$Index = @{}
Get-ChildItem -Path $ModelsDir -Filter *.py | Where-Object { $_.Name -notmatch '^_' } | ForEach-Object {
  ...
  # pata majina yote ya class kwa pattern pana
  foreach ($m in ([regex]'(?m)^\s*class\s+([A-Za-z_][A-Za-z0-9_]*)\s*(?:[\(:])').Matches($txt)) {
    $cls = $m.Groups[1].Value
    if (-not $Index.ContainsKey($cls)) { $Index[$cls] = $mod }
  }
}

# Fallback aliases kwa baadhi ya majina (tutayatumia kama hayapo index)
$AliasFix = @{
...
function Resolve-Class([string]$name) {
  if ($Index.ContainsKey($name)) {
    return @{ Class=$name; Module=$Index[$name] }
  }
  if ($AliasFix.ContainsKey($name)) {
    return $AliasFix[$name]
  }
  # guess snake_case file then verify class ndani
  ...
  if (Test-Path $guessPath) {
    ...
    if ((New-ClassRegex $name).IsMatch($txt)) {
      return @{ Class=$name; Module=$guess }
    }
  }
  # prefetch: search models/*.py vyovyote vile
  $hit = Get-ChildItem -Path $ModelsDir -Filter *.py |
         Select-String -Pattern (New-ClassRegex $name).ToString() -SimpleMatch:$false -ErrorAction SilentlyContinue |
         Select-Object -First 1
  if ($hit) {
    $mod = [IO.Path]::GetFileNameWithoutExtension($hit.Path)
    $Index[$name] = $mod
    return @{ Class=$name; Module=$mod }
  }
  return $null
}
...
function Make-Imports([string[]]$Items) {
  ...
  foreach ($raw in $Items) {
    ...
    $res = Resolve-Class $name
    if ($res) {
      $cls = $res.Class; $mod = $res.Module
      if ($alias) { $out += "from backend.models.$mod import $cls as $alias" }
      else        { $out += "from backend.models.$mod import $cls" }
    } else {
      # acha original ikiwa hatukupata
...
foreach ($f in $targets) {
  ...
  # track unresolved names (bado na 'from backend.models import X')
  $left = ($text -split "`n") | Where-Object { $_ -match 'from\s+backend\.models\s+import\s+' }
  if ($left) { $unresolved[$f.FullName] = $left }
}

Write-Host ("Done. Patched files: {0}" -f $fixed) -ForegroundColor Cyan

if ($unresolved.Count -gt 0) {
  Write-Host "Still unresolved (needs manual check):" -ForegroundColor Yellow
  foreach ($k in $unresolved.Keys) {
    Write-Host " - $k" -ForegroundColor Yellow
    $unresolved[$k] | ForEach-Object { Write-Host ("     " + $_) -ForegroundColor DarkYellow }
  }
} else {
